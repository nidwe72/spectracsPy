...
> the click-through: the gauge renders as a **labeled metric-grid ROW** — caption **"verdict (S/Q ratio)"** is
...
> (label left, gauge in the value column). 236 tests green. **Only G9 full rig sign-off remains.**
...
| `bandLeft`, `bandRight` | axis endpoints in metric units (as-built `4.5`, `2.0`; may descend). Marker is linear between them. |
...
| `caption: str?` | the left-column label chip — as-built `"verdict (S/Q ratio)"`. |
...
class RoastGaugeView(VerdictGaugeView):
    ...
    _THRESHOLDS = [2.8]
    _BAND_LEFT, _BAND_RIGHT = 4.5, 2.0
...
- **`LABEL`** — the verdict **pill**: class label on a rounded, class-coloured badge.
- **`BAND`** — the OKLab gradient band (from `gradientStops`) + a marker line/dot at `positionOf(value)`;
  optional faint threshold ticks.
- **`SWATCH`** — a solid chip = `gradientColorAt(value)` (the oil-coloured square) **with the metric value
  printed on it** in `valueColor` (white for roast), formatted to **2 dp** (`3.78`, D8-value-format). The chip
  must be sized to hold the number.
- **`VALUE`** — the numeric metric as standalone text (`3.78`, 2 dp), in default app ink (not `valueColor` —
  that one is specifically the on-swatch overlay).

Composed per use-site: the Evaluation gauge = `BAND | LABEL | SWATCH` (the value rides on the swatch); the
LIMS badge = `LABEL | SWATCH`; a bare chip = `SWATCH`. `render` **serializes as a list of the on component
names** (e.g. `["band","label","swatch"]`) so the persisted setup stays human-readable in `workflow.json`
(§8.3b).

**As-built layout — a labeled metric-grid ROW** (Edwin, rig 2026-07-24). The gauge is NOT a standalone block:
`visitGauge` renders it as a row **in the shared metric grid** so it lines up with the metric fields — the
`caption` ("verdict (S/Q ratio)") is the gray label chip in col 0 (a `TooltipPageLabel`, identical to the
metric labels), the band + swatch + pill sit in col 1 (the value column). So `visitGauge` **participates in**
the metric grid (does *not* flush it).

- **Qt** (`spectracsPy/.../workflow/render/GaugeWidget.py` + `QtWorkflowRenderer.visitGauge`) — the col-1
  `GaugeWidget`: band via a `QLinearGradient` from `gradientStops` (painted in `_GaugeBandBar`, **square
  corners**, top-aligned with the label chip); the **swatch** and the **verdict pill** share a rounded radius
  and a **fixed height** (`_CHIP_HEIGHT`); the pill is **Expanding** — it consumes the remaining width so its
  right edge aligns with the value fields. The caption is drawn by the host chip, not the widget.
- **matplotlib** (`spectracsPy-core/.../report/MatplotlibWorkflowRenderer.py`) — mirrors the row: caption at
  the left, band (`imshow` of `gradientStops`) + `axvline`+dot marker + swatch (`Rectangle` with the value) +
  pill (a rounded **text-bbox**, not `FancyBboxPatch` — which bow-tied in a wide/short axes) in the value
  column. Reserve `__H_GAUGE_IN`.
...
- **D8-endpoints (RESOLVED — band 4.5 → 2.0).** Deliberately wider than the data to **anticipate very green and
  very brown oils** (Edwin). *Left endpoint moved 4.0 → **4.5** on the rig (2026-07-24)* for more green headroom.
  A run above 4.5 has its **marker line clamped to the left edge** while its **value text stays the true number**
  and its swatch colour clamps to the left anchor. Only the marker position clamps; the number never lies. (RD#5.)
