...
### 9.8 Follow-up phases (C6–C11) — live-review fixes

Found by running the app and reviewing real screens (each committed, offscreen-gallery verified).
**All DONE.**

| Phase | Fix | Detail |
|-------|-----|--------|
| C6 | Uniform framed-panel inner padding (P=`M`=12) | Panels had 3 padding regimes (PageWidget `0,S,S,S` with left=0; view-code Qt-default 11; outer 12). Standardised: bordered panels get symmetric `(M,M,M,M)`; the borderless topmost gets `(0,S,0,S)`. Added `Metrics.applyPanelPadding` helper for the 6 view-code framed panels. |
| C7 | Fix nested-field stair-step (two-level alignment) | Padding compounded per nesting level (labels at x=35/49/63). Only **bordered** panels indent: borderless containers + `createLabeledComponent` + nav boxes use horizontal-0. Result: page-level content shares one edge (~26), panel-internal fields share another (~41). |
| C8 | Untitled nested containers borderless + holder margins | An untitled nested container drew a frame that double-lined with the `QStackedWidget`; tagged it `plain`. Zeroed intermediate `QWidget` holder margins (`createMainWidget`/`createButtonsPanel`) so they stop insetting content. |
| C9 | Tab content = bordered card | `QTabWidget::pane` gets a `1px` border + `M` padding so tab content is a padded card. Wired `Metrics.M` into the QSS via a `panelPadding` format arg. |
| C10 | Strip the tab-internal stacked-widget border | The `QStackedWidget` that `QTabWidget` hosts pages in was drawing the generic border, doubling with the pane card + the video panel. `QTabWidget QStackedWidget { border: none }`. Tab card + video panel keep their borders. |
| C11 | Align section-label headings with their fields | `QGroupBox[sectionLabel]::title` left → 0 so the heading sits above the field's left edge, not the bordered-panel title inset. |

### 9.9 Decisions taken during live review
- **Settings frames KEPT** — Acquisition/Infos stay bordered multi-child groups (their buttons are
  inset vs the full-width borderless-section buttons); the user chose grouping over full alignment.
- **Calibration frame model** — tab card bordered (outer), content panels (video, ROI) bordered, the
  intermediate stacked-widget frame removed. Tab content padded inside the card.
- **Spacing token reaches the QSS** — `Metrics.M` is now injected into the generated stylesheet
  (`panelPadding`), so QTabWidget padding is tokenised, not a literal.
