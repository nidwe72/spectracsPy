...
|---|---|:---:|---|
| **P4c · Usage-crash fixes** | **ROOT CAUSE (2026-07-03):** Qt-for-Android supports only ONE top-level window/EGL surface, so **every top-level `QDialog`/`QMessageBox` aborts** the app (`SIGABRT` — "Failed to acquire deadlock protector for QAndroidPlatformOpenGLWindow::eglSurface()"). Confirmed via the login path: account icon → `ServiceLoginDialog` (QDialog) → crash. FIX: on Android, embed dialogs/forms as pages in the main `QStackedWidget` nav (not separate windows) and replace `QMessageBox` with in-window banners. **DONE for login (2026-07-03):** `LoginViewModule` (in-window page, index 13) replaces the dialog on Android; account-button logout also goes direct (QMenu popup crashes too). **✅ ALL `QMessageBox` CONVERTED 2026-07-04:** new reusable **`InWindowDialog`** (`view/application/widgets/`) — a scrim+card overlay INSIDE the top-level window (no second EGL surface) with a nested event loop for a synchronous `confirm()`/`notify()` result; drop-in for `QMessageBox.question/warning/information`, used on **both** desktop and Android (one themed code path). All 13 call sites swept (wizard Cancel/Delete, measurements-list + user-list Delete, user save, calibration warnings, login-failed); verified on device. **REMAINING:** any non-`QMessageBox` top-level windows (e.g. `QFileDialog` handled separately as P4g; `QMenu` popups); delta-E `_bz2` (reimplement `SpectralColorUtil.getColorDifference` in numpy, or wire arm64 libbz2). **NB:** the virtual-spectrometer "Set image folder" is **NOT** a P4c widget-dialog crash — on-device it opens the native SAF picker fine; the app is then reclaimed while backgrounded. Tracked separately as **P4g / §3.2**. | device | no crash navigating / logging in / running the virtual flow |
...
- **"Detect peaks" crash (49/50)** — `SpectrometerWavelengthCalibrationLogicModule._processSpectralLine`
  dispatched via `getattr(self, f"_processSpectralLine{colorName}")`. The enum is `(str, Enum)`; on
  Python 3.10 (desktop) `f"{member}"` is the value (`MERCURY_MANGO_GREEN`), but on **Android's Python
  3.11** the enum `__str__`/`__format__` change makes it `ClassName.MEMBER` → wrong method →
  `AttributeError`. Fix: `.name` (version-independent). ⚠️ general 3.10→3.11 hazard for any
  `str(enum)`/`f"{enum}"` that expects the value.
- **P4c dialog category (all `QMessageBox`)** — see the P4c row: replaced by the reusable
  `InWindowDialog`.
