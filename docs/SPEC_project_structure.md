...
Status: **S0 IMPLEMENTED (2026-07-16); S1a–S5 DESIGN** (spec-first; implement on explicit request only). Source:
...
| **S1a** | **CHARACTERISE — no production change.** Pin the **current** QColor-based behaviour of `SpectralColorUtil.wavelengthToColor` / `hueSimilarity` / `channelDominance` with tests, against real values. **Worth doing even if S1b never happens** (§4d). | `tests/` only | tests pass **against today's QColor code**, and are specific enough to fail if the maths moves — incl. the **achromatic case** (§4d) |
